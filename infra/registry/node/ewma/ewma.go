...
func (b *Builder) Build(n registry.Node) registry.WeightedNode {
	s := &Node{
		Node:         n,
		lag:          0,
		success:      1000,
...
func (n *Node) Pick() registry.DoneFunc {
	...
	return func(_ context.Context, di registry.DoneInfo) {
		if swapped {
		...
		if di.Err != nil {
			...
			if werror.Is(context.DeadlineExceeded, di.Err) || werror.Is(context.Canceled, di.Err) ||
				werror.Code(di.Err) == 503 || werror.Code(di.Err) == 504 || errors.As(di.Err, &netErr) {
				success = 0
			}
		}
