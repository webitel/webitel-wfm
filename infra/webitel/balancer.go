...
func init() {
	if registry.GlobalSelector() == nil {
	...
	b := base.NewBalancerBuilder(
		balancerName,
		&balancerBuilder{
			builder: registry.GlobalSelector(),
		},
...
func (p *balancerPicker) Pick(info balancer.PickInfo) (balancer.PickResult, error) {
	// TODO: add filter from info.Ctx
	var filters []registry.NodeFilter
	n, done, err := p.selector.Select(info.Ctx, registry.WithNodeFilter(filters...))
	if err != nil {
	...
	return balancer.PickResult{
		...
		Done: func(di balancer.DoneInfo) {
			done(info.Ctx, registry.DoneInfo{
				Err:           di.Err,
