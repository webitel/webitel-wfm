...
//
// It is useful to ensure scany works after adding a field to the database,
...
//
// Doing this "in the other direction", see
...
func Wildcard(v any) string {
	...
	// The logic below based on strings.Join, but avoids column ambiguity.
	if len(elems) == 0 {
		return ""
	}
...
func fields(rv reflect.Type) []string {
	...
	var cs []column
	for name, i := range GetColumnToFieldIndexMap(rv) {
		cs = append(cs, column{
			indices: i,
			name:    name,
		})
