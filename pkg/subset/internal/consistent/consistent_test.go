...
func BenchmarkCycle(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		x.Add(member("foo" + strconv.Itoa(i)))
		x.Remove(member("foo" + strconv.Itoa(i)))
	}
}
...
func BenchmarkCycleLarge(b *testing.B) {
	x := New[member]()
	for i := 0; i < 10; i++ {
		x.Add(member("start" + strconv.Itoa(i)))
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		x.Add(member("foo" + strconv.Itoa(i)))
...
func TestConcurrentGetSet(t *testing.T) {
	x := New[member]()
	x.Set([]member{"abc", "def", "ghi", "jkl", "mno"})
